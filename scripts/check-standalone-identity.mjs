...
for (const path of packages) {
	...
	if (manifest.version !== "0.1.0") failures.push(`${path}: expected version 0.1.0, found ${manifest.version}`);
	...
	for (const section of ["dependencies", "devDependencies", "peerDependencies", "optionalDependencies"]) {
		for (const [name, range] of Object.entries(manifest[section] ?? {})) {
			...
			if (name.startsWith("@reitaard/recode-") && range !== "^0.1.0") {
				failures.push(`${path}: internal dependency ${name} must use ^0.1.0, found ${range}`);
			}
		}
	}
}
...
console.log("standalone identity check passed for seven packages at 0.1.0");
