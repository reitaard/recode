...
Use the [17-slice staged plan](plans/STAGED-TRANSFER.md): freeze and transfer coding-agent first, neutralize its inherited identity, then transfer dependency packages, minimal root infrastructure, and certification/release files. Each copy requires its own scoped ledger, dry run, and Creator approval. Preserve provenance and leave the source repository unchanged.
