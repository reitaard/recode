# Recode

A coding harness built around Aizen, reusable agent runtimes, workers, durable memory, code intelligence, and Maestro.

The `recode` command runs Aizen, Maestro, and Recode-specific features. Recode `0.1.6` also provides a `pi` compatibility command and a Termux/aarch64 release candidate. The GitHub release uses one low-noise bundle containing the seven package tarballs, Termux package, checksums, provenance, and release notes. Existing Pi ecosystem installation instructions work unchanged:

```powershell
pi install npm:pi-better-harness
pi list
pi update
```

A global Recode installation that exposes `pi` replaces or shadows any existing upstream `pi` command. npm publication remains disabled; releases are distributed through GitHub.

See [the documentation index](docs/INDEX.md) and [changelog](CHANGELOG.md).
