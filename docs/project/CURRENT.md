...
- Seven private `@reitaard/recode-*` packages use the synchronized `0.1.3` candidate version train.
...
- GitHub `v0.1.0` and `v0.1.2` are published. The `v0.1.3` candidate closes legacy-extension, ANSI-rendering, changelog, and global-migration gaps and remains unreleased until certification and explicit remote approval.
