...
Use the [18-slice staged plan](plans/STAGED-TRANSFER.md). Package transfer, root bootstrap, and known-defect repair are complete through Slice 13. All seven packages build; production audit reports zero vulnerabilities; shrinkwrap, telemetry documentation, and maintained examples have active drift/compile checks. The coding-agent suite passes with 2,086 active tests, zero failures, and 52 explicit skips. Slice 14 is active; Slices 14–17 certify and prepare the repository; Slice 18 performs the explicitly approved GitHub push, `v0.1.0` tag, and GitHub Release. Each future source copy remains manifest-scoped. Preserve provenance and leave the source repository unchanged.
