...
/** Canonical modified sequences retained as a public Pi-compatibility contract. */
...
export interface Terminal {
	...
	// Current keyboard protocol and observed canonical Recode sequences
...
export interface ProcessTerminalOptions {
	/** Capture every terminal write to this file. Parent directories are created lazily. */
	writeLogPath?: string;
}

function resolveWriteLogPath(configuredPath: string): string {
	if (!configuredPath) return "";
	try {
		if (fs.statSync(configuredPath).isDirectory()) {
			const now = new Date();
			const timestamp = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-${String(now.getDate()).padStart(2, "0")}_${String(now.getHours()).padStart(2, "0")}-${String(now.getMinutes()).padStart(2, "0")}-${String(now.getSeconds()).padStart(2, "0")}`;
			return path.join(configuredPath, `tui-${timestamp}-${process.pid}.log`);
		}
	} catch {
		// Not an existing directory; use the configured value as a file path.
	}
...
export function resolveEscapeTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
	const configured = Number(env.RECODE_TUI_ESC_TIMEOUT);
...
export class ProcessTerminal implements Terminal {
	...
	private readonly writeLogPath: string;
	private writeLogInitialized = false;

	constructor(options: ProcessTerminalOptions = {}) {
		this.writeLogPath = resolveWriteLogPath(options.writeLogPath ?? process.env.PI_TUI_WRITE_LOG ?? "");
	}
	...
	write(data: string): void {
		...
		if (!this.writeLogInitialized) {
			this.writeLogInitialized = true;
			try {
				fs.mkdirSync(path.dirname(this.writeLogPath), { recursive: true });
			} catch {
