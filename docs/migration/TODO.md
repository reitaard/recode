...
Use the [17-slice staged plan](plans/STAGED-TRANSFER.md). Slices 1–8 are complete: coding-agent, telemetry, AI, and Agent core were scoped, transferred, identity-neutralized, and statically audited. Slice 9 TUI is next, followed by services/storage, minimal root infrastructure, and certification/release files. Each copy requires its own scoped ledger, dry run, and Creator approval. Preserve provenance and leave the source repository unchanged.
