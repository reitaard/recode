...
describe.sequential("OAuthAuth adapters", () => {
	...
	it("keeps legacy OAuth exports available during root credential migration", () => {
		expect(extensionOAuthCompatibility).toHaveProperty("loginAnthropic");
	...
	it("openrouter derives the api key and keeps the permanent credential on refresh", async () => {
		...
		expect(await openRouterOAuth.refresh(credential, signal)).toBe(credential);
	});
