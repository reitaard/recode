...
export type CliIdentity = "recode" | "pi";

/** Normalize only the compatibility behavior that differs by executable name. */
export function normalizeCliArgs(identity: CliIdentity, args: readonly string[]): string[] {
	if (identity === "pi" && args.length === 1 && args[0] === "update") {
		return ["update", "--extensions"];
	}
	return [...args];
}

export async function runCli(identity: CliIdentity, rawArgs: readonly string[] = process.argv.slice(2)): Promise<void> {
	process.title = identity === "pi" ? "pi" : APP_NAME;
	process.env.PI_CODING_AGENT = "true";
	process.env.AI_AGENT = "pi";
	process.emitWarning = (() => {}) as typeof process.emitWarning;

	const args = normalizeCliArgs(identity, rawArgs);
