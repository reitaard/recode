...
Use the [18-slice staged plan](plans/STAGED-TRANSFER.md). Package transfer, root bootstrap, and known-defect repair are complete through Slice 13. All seven packages build; production audit reports zero vulnerabilities; shrinkwrap, telemetry documentation, and maintained examples have active drift/compile checks. The full workspace checks, dependency-ordered builds, deterministic tests, examples, and public root/subpath import smoke tests pass; production audit reports zero vulnerabilities. Slices 14–15 are complete: all seven inspected tarballs install together with lifecycle scripts disabled in isolation, maintained exports and npm-generated CLI shims pass, shrinkwrap/assets are present, and no legacy package dependency leaks remain. Slice 16 is complete for the standalone Windows x64/JavaScript boundary: native addons are omitted, the safe fallback passes, and all other platforms/native/external integrations remain explicitly uncertified or opt-in. Slice 17 is active and prepares the final local public-readiness checkpoint; Slice 18 performs the explicitly approved GitHub push, `v0.1.0` tag, and GitHub Release. Each future source copy remains manifest-scoped. Preserve provenance and leave the source repository unchanged.
