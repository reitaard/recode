...
import { type CredentialStore, InMemoryModelsStore } from "@reitaard/recode-ai";
...
async function createRuntime(credentials: CredentialStore): Promise<ModelRuntime> {
...
describe("auth check command", () => {
	...
	test("does not treat an unresolved stored environment reference as configured", async () => {
		...
		const runtime = await createRuntime(AuthStorage.create(authPath));
	...
	test("treats malformed auth state as unconfigured", async () => {
		const authPath = join(tempDir, "auth.json");
	...
	test("initializes a missing auth file for the file-backed credential store", async () => {
