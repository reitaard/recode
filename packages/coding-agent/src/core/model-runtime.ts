...
import * as builtinProviderCatalog from "@reitaard/recode-ai/providers/all";
...
export class ModelRuntime implements Models {
	...
	static async create(options: CreateModelRuntimeOptions = {}): Promise<ModelRuntime> {
		...
		const providers = builtinProviderCatalog.builtinProviders().map((provider) =>
			provider.id === "radius" || !options.catalogBaseUrl
				? provider
	...
	listCredentials(_options?: AuthOperationOptions): Promise<readonly CredentialInfo[]> {
