...
/** Optional bounded diagnostics for renderer performance investigations. */
...
export const VIEWPORT_TUI = Symbol.for("@reitaard/recode-tui/viewport");
...
export abstract class TuiBase extends Container implements TUI {
	...
	private readonly slowRenderThresholdMs: number | undefined;
	...
	constructor(terminal: Terminal, showHardwareCursor?: boolean, logDirectory?: string | TuiDiagnosticsOptions) {
		super();
		this.terminal = terminal;
		const diagnostics = typeof logDirectory === "object" ? logDirectory : undefined;
		this.logDirectory =
			(typeof logDirectory === "string" ? logDirectory : undefined) ??
			process.env.PI_CODING_AGENT_DIR ??
			path.join(os.homedir(), ".pi", "agent");
		this.diagnosticsLogPath = diagnostics?.logPath;
		this.slowRenderThresholdMs = diagnostics?.slowRenderThresholdMs;
	...
	protected getRenderDiagnosticFields(): Record<string, number> {
		return {};
	}
	...
	private performRender(): void {
		...
		if (this.slowRenderThresholdMs !== undefined && elapsedMs >= this.slowRenderThresholdMs) {
			writeTuiDiagnostic(this.diagnosticsLogPath, "slow-render", {
				elapsedMs: Math.round(elapsedMs),
				...this.getRenderDiagnosticFields(),
			});
		}
	}
