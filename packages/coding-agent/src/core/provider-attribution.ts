import type { Api, Model, ProviderHeaders } from "@reitaard/recode-ai";
...
function getDefaultAttributionHeaders(
	...
	if (isOpenRouterModel(model)) {
		return {
			"HTTP-Referer": "https://github.com/reitaard/recode",
			"X-OpenRouter-Title": "Recode",
	...
	if (isNvidiaNimModel(model)) {
		return {
			"X-BILLING-INVOKE-ORIGIN": "Recode",
		};
	}

	if (isCloudflareModel(model)) {
		return {
			"User-Agent": "recode-coding-agent",
		};
	}
...
function getSessionHeaders(model: Model<Api>, sessionId: string | undefined): Record<string, string> | undefined {
	...
	return { "x-opencode-session": sessionId, "x-opencode-client": "Recode" };
}
