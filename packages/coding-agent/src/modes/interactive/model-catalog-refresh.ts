import type { ModelsRefreshResult } from "@reitaard/recode-ai";
	...
	refresh(modelRuntime: ModelCatalogRuntime, signal: AbortSignal): Promise<ModelsRefreshResult> {
		...
		if (!active) {
			...
			const promise = raceWithAbortSignal(operation, controller.signal).finally(() => {
				if (this.activeByRuntime.get(modelRuntime) === created) this.activeByRuntime.delete(modelRuntime);
			});
		...
		return raceWithAbortSignal(active.promise, signal).finally(() => {
			active.waiters--;
			if (active.waiters === 0 && this.activeByRuntime.get(modelRuntime) === active) active.controller.abort();
		});
	}
}
