import type { Api, CredentialInfo, Model } from "@reitaard/recode-ai";
...
export async function resolveCredentialForPrint(
	...
	if (cliProvider) {
		...
		if (cliModel) {
			const resolved = resolveCliModel({ cliProvider: provider.id, cliModel, modelRegistry: modelRuntime });
