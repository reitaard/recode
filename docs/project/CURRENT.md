...
- Seven private `@reitaard/recode-*` packages use the synchronized `0.1.4` candidate version train.
...
- GitHub `v0.1.0` and `v0.1.2` are published. The `v0.1.4` candidate closes the status-layout and installer-path gaps and remains unreleased until certification and explicit remote approval.
