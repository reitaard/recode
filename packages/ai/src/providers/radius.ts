...
export function radiusProvider(options: RadiusProviderOptions = {}): Provider<"pi-messages"> {
	...
	const configuredGateway = options.gateway ?? DEFAULT_RADIUS_GATEWAY;
	const gateway = configuredGateway ? normalizeRadiusGatewayUrl(configuredGateway) : "";
	...
	return {
		...
		refreshModels: async (context) => {
			...
			if (!gateway) {
				throw new Error("Radius requires an explicit gateway URL");
			}
