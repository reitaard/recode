...
export function classifyCurrentInstallation(npmCommand?: string[]): InstallationClassification {
	...
	return classifyInstallationSnapshot({
		...
		hasReleaseProvenance:
			metadata.name === "@reitaard/recode-coding-agent" &&
			typeof metadata.recode?.sourceCommit === "string" &&
			/^[0-9a-f]{40}$/.test(metadata.recode.sourceCommit),
...
export function getSelfUpdateUnavailableInstruction(
	...
	if (method === "bun-binary") {
		return "Binary self-update is disabled until a certified Recode release channel is configured.";
	}
...
export const PACKAGE_NAME: string = pkg.name || "@reitaard/recode-coding-agent";
export const APP_NAME: string = piConfigName || "pi";
export const APP_TITLE: string = piConfigName ? APP_NAME : "π";
export const CONFIG_DIR_NAME: string = pkg.piConfig?.configDir || ".pi";
export const VERSION: string = pkg.version || "0.0.0";

// Preserve the established environment contract while the visible CLI is renamed.
...
/** Get the share viewer URL for a gist ID when an explicit compatible viewer is configured. */
export function getShareViewerUrl(gistId: string): string {
	const baseUrl = process.env.PI_SHARE_VIEWER_URL;
	if (!baseUrl) throw new Error("Session sharing is disabled until PI_SHARE_VIEWER_URL is configured");
