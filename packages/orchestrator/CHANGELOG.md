...
- Added bounded completion-queue records with generation-safe claims, idempotent acknowledgement, and explicitly untrusted Aizen handoffs at idle turn boundaries. Intended restart delivery remains uncertified because the current focused recovery test fails.
...
- Wired real child terminal transitions to one completion record before parent delivery and acknowledgement; crash/restart durability is not currently certified.
