...
export interface CreateModelsOptions {
	...
	/** Final Recode request hook after provider auth and request options are assembled. */
	prepareRequest?: (
		model: Model<Api>,
		options: ProviderRequestOptions | undefined,
	) => Promise<ProviderRequestOptions | undefined>;
}
...
class ModelsImpl implements MutableModels {
	...
	private async applyAuth<TOptions extends ProviderRequestOptions & ModelsRequestTransforms>(
		...
		if (!resolution) {
			...
			const requestOptions = await this.prepareRequest(model, options);
			return {
				requestModel: model,
				requestOptions: requestOptions as Omit<TOptions, "transformHeaders"> & ProviderRequestOptions,
			};
		}
		...
		let requestOptions = { ...providerOptions, apiKey, headers, env } as Omit<TOptions, "transformHeaders"> &
			ProviderRequestOptions;
		if (this.prepareRequest) {
			requestOptions = (await this.prepareRequest(requestModel, requestOptions)) as typeof requestOptions;
		}
