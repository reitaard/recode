...
import { AuthStorage, type OAuthCredential } from "@reitaard/recode-coding-agent";
...
export function getRadiusUrl(): string {
	const value = process.env.PI_RADIUS_URL || DEFAULT_RADIUS_URL;
	if (!value) throw new Error("Radius requires an explicit PI_RADIUS_URL");
	return value;
}
...
export function isRadiusEnabled(): boolean {
	const hasEndpoint = !!process.env.PI_RADIUS_URL || !!process.env.PI_RADIUS_ORCHESTRATOR_URL;
	return hasEndpoint && (!!getStoredRadiusCredential()?.access || !!process.env.PI_RADIUS_API_KEY);
}
