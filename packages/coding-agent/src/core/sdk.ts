...
import { Agent, type AgentMessage, type ThinkingLevel } from "@reitaard/recode-agent-core";
import { clampThinkingLevel, type Message, type Model, streamSimple } from "@reitaard/recode-ai/compat";
 ...
 *
 * // With explicit model
 * import { getModel } from '@reitaard/recode-ai';
 * const { session } = await createAgentSession({
...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	const defaultActiveToolNames: ToolName[] = [
		...
		"edit",
		"write",
		...(Object.keys(loadLspConfig(cwd, agentDir, settingsManager.getLspSettings()).servers).length > 0
			? (["lsp"] as const)
			: []),
	];
